# danmu_api（lilixu3 维护版）

> 一个可自托管的弹幕聚合 API 服务：支持多平台弹幕直接获取，兼容弹弹play 的搜索/匹配/详情/弹幕接口规范，并内置 Web UI 后台（配置/日志/接口调试/推送弹幕/系统管理等）。
>
...
- **Web UI 后台**：配置预览、日志查看、接口调试、推送弹幕、系统管理等（详情见 `danmu_api/ui/README.md`）。
...
| `SEARCH_CACHE_MINUTES` | 搜索缓存分钟数（`0` 表示不缓存） |
| `COMMENT_CACHE_MINUTES` | 弹幕缓存分钟数（`0` 表示不缓存） |
