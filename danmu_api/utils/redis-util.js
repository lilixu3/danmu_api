...
export async function pingRedis() {
  ...
  log("debug", `[redis] 开始发送 PING 请求:`, url);
  try {
...
export async function getRedisKey(key) {
  ...
  log("debug", `[redis] 开始发送 GET 请求:`, url);
  try {
    const response = await fetch(url, {
      method: 'GET',
      headers: {
...
export async function setRedisKey(key, value) {
  ...
  if (globals.lastHashes[key] === currentHash) {
    log("debug", `[redis] 键 ${key} 无变化，跳过 SET 请求`);
    return { result: "OK" }; // 模拟成功响应
  }

  const url = `${globals.redisUrl}/set/${key}`;
  log("debug", `[redis] 开始发送 SET 请求:`, url);
  try {
    const response = await fetch(url, {
      method: 'POST',
      headers: {
...
export async function setRedisKeyWithExpiry(key, value, expirySeconds) {
  ...
  // 检查值是否变化
  if (globals.lastHashes[key] === currentHash) {
    log("debug", `[redis] 键 ${key} 无变化，跳过 SETEX 请求`);
    return { result: "OK" }; // 模拟成功响应
  }

  const url = `${globals.redisUrl}/set/${key}?EX=${expirySeconds}`;
  log("debug", `[redis] 开始发送 SETEX 请求:`, url);
  try {
    const response = await fetch(url, {
      method: 'POST',
      headers: {
    ...
    const result = await response.json();
    globals.lastHashes[key] = currentHash; // 更新哈希值
    log("debug", `[redis] 键 ${key} 更新成功（带过期时间 ${expirySeconds}s）`);
    return result;
  } catch (error) {
...
export async function runPipeline(commands) {
  ...
  log("debug", `[redis] 开始发送 PIPELINE 请求:`, url);
  try {
    const response = await fetch(url, {
      method: 'POST',
      headers: {
...
export async function getRedisCaches() {
  if (!globals.redisCacheInitialized) {
    try {
      log("debug", 'getRedisCaches start.');
      ...
      if (lastSelectMapData && typeof lastSelectMapData === 'object') {
        ...
        log("debug", `Restored lastSelectMap from Redis with ${globals.lastSelectMap.size} entries`);
      }
      ...
      log("debug", 'getRedisCaches completed successfully.');
    } catch (error) {
...
export async function updateRedisCaches() {
  try {
    ...
    if (commands.length > 0) {
      log("debug", `Updating ${commands.length} changed keys: ${updates.map(u => u.key).join(', ')}`);
      const results = await runPipeline(commands);
      ...
      if (failureCount === 0) {
        ...
        log("debug", `Redis update completed successfully: ${successCount} keys updated`);
      } else {
        log("warn", `Redis update partially failed: ${successCount} succeeded, ${failureCount} failed`);
      }
    } else {
      log("debug", 'No changes detected, skipping Redis update.');
    }
  } catch (error) {
