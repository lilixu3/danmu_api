...
test("hanjutv2 会裁剪离群尾部时间戳", async () => {
  const source = new Hanjutv2Source();
  const calls = [];

  const normalDanmus = Array.from({ length: 1000 }, (_, i) => ({
    id: i + 1,
    t: 1000 + (i * 100),
    con: String(i + 1),
  }));
  const outlierDanmus = [
    { id: 1001, t: 900000, con: "x1" },
    { id: 1002, t: 1500000, con: "x2" },
  ];
  const allDanmus = [...normalDanmus, ...outlierDanmus];

  source.getDanmuTailTrimConfig = () => ({
    minSamples: 100,
    triggerGapMs: 300000,
    keepTailBufferMs: 10000,
    maxTailRatio: 0.02,
  });
  source.getDanmuMaxPages = () => 5;
  source.getDanmuMaxAxis = () => 100000000;
  source.createLiteSession = async () => ({ uid: "test_uid", headers: {} });
  source.requestLiteApi = async (pathname, query) => {
    calls.push({ pathname, query: { ...query } });
    if (calls.length === 1) {
      return { bulletchats: allDanmus.slice(0, 600), more: 1, nextAxis: 50000, lastId: 600 };
    }
    if (calls.length === 2) {
      return { bulletchats: allDanmus.slice(600), more: 0, nextAxis: 100000000, lastId: 1002 };
    }
    return { bulletchats: [], more: 0, nextAxis: query.fromAxis, lastId: query.prevId };
  };

  const danmus = await source.getEpisodeDanmuByLite("EP_OUTLIER");

  assert.equal(calls.length, 2);
  assert.equal(danmus.length, 1000);
  assert.equal(Math.max(...danmus.map((item) => Number(item.t || 0))), 100900);
});

test("hanjutv2 分段上限使用清洗后最大时间", async () => {
  const source = new Hanjutv2Source();
  source.getEpisodeDanmuByLite = async () => [{ id: 1, t: 4200000, con: "ok" }];

  const res = await source.getEpisodeDanmuSegments("xw:EP_SEG");

  assert.equal(res.segmentList.length, 1);
  assert.equal(res.segmentList[0].url, "xw:EP_SEG");
  assert.equal(res.segmentList[0].segment_end, 4260);
});

test("hanjutv 会裁剪离群尾部时间戳", () => {
  const source = new HanjutvSource();
  const normalDanmus = Array.from({ length: 1200 }, (_, i) => ({
    id: i + 1,
    t: 1000 + (i * 120),
    con: String(i + 1),
  }));
  const outliers = [
    { id: 1201, t: 900000, con: "x1" },
    { id: 1202, t: 1800000, con: "x2" },
  ];

  const res = source.trimDanmuTailOutliers([...normalDanmus, ...outliers]);

  assert.equal(res.trimmedCount, 2);
  assert.equal(Math.max(...res.comments.map((item) => Number(item.t || 0))), 144880);
});

test("hanjutv 分段上限使用清洗后最大时间", async () => {
  const source = new HanjutvSource();
  source.getEpisodeDanmuByHxq = async () => [{ id: 1, t: 3600000, con: "ok" }];

  const res = await source.getEpisodeDanmuSegments("EP_HXQ_SEG");

  assert.equal(res.segmentList.length, 1);
  assert.equal(res.segmentList[0].url, "EP_HXQ_SEG");
  assert.equal(res.segmentList[0].segment_end, 3660);
});
