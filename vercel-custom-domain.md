...
![Vercel 添加域名](./guide-assets/custom-domain/vercel-add-domain.jpeg)
...
| 子域名 `www.example.com` | `CNAME`，常见值是 `cname.vercel-dns.com` 或 Vercel 当前给出的值 | 去 Cloudflare 添加一条 `CNAME` |
| 某些情况 | `TXT` 验证记录 | 只有 Vercel 明确要求时才加 |

![Vercel 官方 DNS 提示](./guide-assets/custom-domain/vercel-official-dns-form.png)

如果 Vercel 明确要求你加 `TXT`，照着它页面显示的名字和值去填：

![Vercel 官方 TXT 提示](./guide-assets/custom-domain/vercel-official-txt-records.png)
...
![Cloudflare 添加域名](./guide-assets/custom-domain/cloudflare-add-domain.jpeg)

Cloudflare 会给你两条 nameserver，先不要关页面，下一步要拿去注册商后台填。

![Cloudflare nameserver](./guide-assets/custom-domain/cloudflare-nameservers.jpeg)
...
![Spaceship 修改 nameserver 入口](./guide-assets/custom-domain/spaceship-change-nameservers.png)

![Spaceship 确认 custom nameserver](./guide-assets/custom-domain/spaceship-confirm-custom-dns.png)
...
不要自己猜，也不要照抄别人的旧教程，**直接照着你自己 Vercel 页面当前显示的值填。**

![Cloudflare DNS 填写示意](./guide-assets/custom-domain/cloudflare-dns-records.jpeg)
...
![Vercel 域名验证成功](./guide-assets/custom-domain/vercel-ssl-issued.jpeg)
